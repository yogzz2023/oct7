...
def plot_track_data(tracks):
    fig, (range_ax, az_ax, el_ax) = plt.subplots(3, 1, figsize=(12, 10))

    for track in tracks:
        track_id = track['track_id']
        measurements = [m[0] for m in track['measurements']]
        times = [m[0][3] for m in track['measurements']]
        ranges = [m[0] for m in measurements]
        azimuths = [m[1] for m in measurements]
        elevations = [m[2] for m in measurements]

        # Plot range and Sf(range) vs time
        range_ax.plot(times, ranges, label=f'Track {track_id} Range')
        range_ax.plot(times, [m[0] for m in track['measurements']], label=f'Track {track_id} Sf(Range)')
        ...
        az_ax.plot(times, azimuths, label=f'Track {track_id} Azimuth')
        az_ax.plot(times, [m[1] for m in track['measurements']], label=f'Track {track_id} Sf(Azimuth)')
        az_ax.set_xlabel('Time')
        az_ax.set_ylabel('Azimuth')
        az_ax.set_title('Azimuth and Sf(Azimuth) vs Time')
        az_ax.legend()

        # Plot elevation and Sf(elevation) vs time
        el_ax.plot(times, elevations, label=f'Track {track_id} Elevation')
        el_ax.plot(times, [m[2] for m in track['measurements']], label=f'Track {track_id} Sf(Elevation)')
        el_ax.set_xlabel('Time')
        el_ax.set_ylabel('Elevation')
        el_ax.set_title('Elevation and Sf(Elevation) vs Time')
        el_ax.legend()

    # Add data tips
    fig.canvas.mpl_connect('pick_event', lambda event: data_tip(event, tracks))
    plt.show()

def data_tip(event, tracks):
    if event.artist in range_ax.lines or event.artist in az_ax.lines or event.artist in el_ax.lines:
        index = event.ind[0]
        for track in tracks:
            if event.artist.get_label() == f'Track {track["track_id"]} Range':
                measurement = track['measurements'][index][0]
                print(f"Track ID: {track['track_id']}")
                print(f"Range: {measurement[0]}")
                print(f"Azimuth: {measurement[1]}")
                print(f"Elevation: {measurement[2]}")
                print(f"Time: {measurement[3]}")
                break
            elif event.artist.get_label() == f'Track {track["track_id"]} Azimuth':
                measurement = track['measurements'][index][0]
                print(f"Track ID: {track['track_id']}")
                print(f"Range: {measurement[0]}")
                print(f"Azimuth: {measurement[1]}")
                ...
                print(f"Time: {measurement[3]}")
                break
            elif event.artist.get_label() == f'Track {track["track_id"]} Elevation':
                measurement = track['measurements'][index][0]
                print(f"Track ID: {track['track_id']}")
                print(f"Range: {measurement[0]}")
                print(f"Azimuth: {measurement[1]}")
